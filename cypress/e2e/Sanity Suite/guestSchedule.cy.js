describe("Guest Schedule Page Tests", () => {
  ...
  it("should display all required Header UI elements", () => {
    ...
    cy.get(".anavio-header").within(() => {
      ...
      cy.get(
        'app-breadcrumbs .last-breadcrumb-item a[href="/settings/schedules/guest-schedule"]'
      )
        .should("be.visible")
        .contains("Guest Schedule");
  ...
  it("should display all required navigation bar elements", () => {
    cy.get(".navigation-bar").within(() => {
      ...
      // Check that Guest Schedule button is the active one
      cy.get("@guestScheduleBtn").should("have.class", "active-route");
